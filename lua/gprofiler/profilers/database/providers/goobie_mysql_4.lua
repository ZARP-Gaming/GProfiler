...
local GoobieMySQL4Provider = setmetatable({}, { __index = BaseProvider })
GoobieMySQL4Provider.__index = GoobieMySQL4Provider
GoobieMySQL4Provider.Name = "goobie_mysql"
GoobieMySQL4Provider.ModuleName = "goobie_mysql_4"
GoobieMySQL4Provider.QueryMethods = {
	"Run", "RunSync", "Execute", "ExecuteSync",
	"Fetch", "FetchSync", "FetchOne", "FetchOneSync",
	"UpsertQuery", "UpsertQuerySync"
}

function GoobieMySQL4Provider:New()
	local provider = BaseProvider.New(self)
	setmetatable(provider, self)
	return provider
end

function GoobieMySQL4Provider:SetupConnectionHook()
	local oldNewConn = goobie_mysql_4.NewConn
	if goobie_mysql_4.oldNewConn then
		oldNewConn = goobie_mysql_4.oldNewConn
	else
		goobie_mysql_4.oldNewConn = oldNewConn
	end

	goobie_mysql_4.NewConn = function(opts, on_connect)
...
function GoobieMySQL4Provider:DetourQueryFunction(objectData)
...
function GoobieMySQL4Provider:WrapQueryMethod(objectData, methodName, obj, ...)
	local args = {...}
	local queryText = args[1]

	local oldFunc = objectData.oldQueryFuncs[methodName]
	if not oldFunc then
		ErrorNoHalt("No old function found for method: " .. methodName)
		return
	end
...
function GoobieMySQL4Provider:EnableSQLProfiling(objectData) end
function GoobieMySQL4Provider:DisableSQLProfiling(objectData) end
function GoobieMySQL4Provider:ExecuteExplainQuery(explainQuery, objectData, queryId)
...
function GoobieMySQL4Provider:GetObjectProfilingData(objectData, callback)
	callback(nil)
end

GProfiler.Database.Providers.Register(GoobieMySQL4Provider)

return GoobieMySQL4Provider
